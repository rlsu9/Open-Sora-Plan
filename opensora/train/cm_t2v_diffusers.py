...
def main(args):
    ...
    teacher_model = Diffusion_models[args.model](
        in_channels=ae_channel_config[args.ae],
        out_channels=ae_channel_config[args.ae],
        # caption_channels=4096,
        # cross_attention_dim=1152,
    ...
    if args.pretrained:
        ...
        if 'safetensors' in args.teacher_model:  # pixart series
            from safetensors.torch import load_file as safe_load
            # import ipdb;ipdb.set_trace()
            pretrained_checkpoint = safe_load(args.teacher_model, device="cpu")
            pretrained_keys = set(list(pretrained_checkpoint.keys()))
            teacher_model_keys = set(list(teacher_model_state_dict.keys()))
            teacher_common_keys = list(pretrained_keys & teacher_model_keys)
            teacher_checkpoint = {k: pretrained_checkpoint[k] for k in teacher_common_keys if model_state_dict[k].numel() == pretrained_checkpoint[k].numel()}
            # if checkpoint['pos_embed.proj.weight'].shape != teacher_model.pos_embed.proj.weight.shape and checkpoint['pos_embed.proj.weight'].ndim == 4:
            #     logger.info(f"Resize pos_embed, {checkpoint['pos_embed.proj.weight'].shape} -> {teacher_model.pos_embed.proj.weight.shape}")
            #     repeat = teacher_model.pos_embed.proj.weight.shape[2]
            #     checkpoint['pos_embed.proj.weight'] = checkpoint['pos_embed.proj.weight'].unsqueeze(2).repeat(1, 1, repeat, 1, 1) / float(repeat)
                # del checkpoint['proj_out.weight'], checkpoint['proj_out.bias']
        else:  # latest stage training weight
            teacher_checkpoint = torch.load(args.teacher_model, map_location='cpu')
            if 'model' in teacher_checkpoint:
                teacher_checkpoint = teacher_checkpoint['model']
        teacher_missing_keys, teacher_unexpected_keys = teacher_model.load_state_dict(teacher_checkpoint, strict=False)
        logger.info(f'missing_keys {len(teacher_missing_keys)} {teacher_missing_keys}, unexpected_keys {len(teacher_unexpected_keys)}')
        logger.info(f'Successfully load {len(teacher_model_state_dict) - len(teacher_missing_keys)}/{len(teacher_model_state_dict)} keys from {args.teacher_model}!')
